        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>Problem Type Id <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="problemTypeId" name="problemTypeId" type="text" class="form-control" placeholder=""
                            value="" readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    <div class="col-md-5">
                        <label> Problem Type Name <span class="text-danger">*</span></label>

                    </div>
                    <div class="col-md-7">
                        <input id="problemTypeName" name="problemTypeName" type="text" class="form-control"
                            placeholder="" value="">
                    </div>
                </div>
                <br />
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateProblemType",
            type: "POST",
            cache: false,
    ...
    $("#problemTypeId").val("<?php echo $this->viewbag['problemType']['problemTypeId']?>");
    $("#problemTypeName").val("<?php echo $this->viewbag['problemType']['problemTypeName']?>");
    $("#active").val("<?php echo $this->viewbag['problemType']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertProblemType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#problemTypeName").val() == "") {
            if (errorMessage == "")
                $("#problemTypeName").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Problem Type Name can not be blank <br/>";
            i = i + 1;
            $("#problemTypeName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnProblemTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
