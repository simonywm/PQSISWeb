        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>EicId <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="eicId" name="eicId" type="text" class="form-control" placeholder="" value=""
                            readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    <div class="col-md-5">
                        <label> Eic Name <span class="text-danger">*</span></label>

                    </div>
                    <div class="col-md-7">
                        <input id="eicName" name="eicName" type="text" class="form-control" placeholder="" value="">
                    </div>
                </div>
                <br />
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateEic",
            type: "POST",
            cache: false,
    ...
    $("#eicId").val("<?php echo $this->viewbag['eic']['eicId']?>");
    $("#eicName").val("<?php echo $this->viewbag['eic']['eicName']?>");
    $("#active").val("<?php echo $this->viewbag['eic']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertEic",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#eicName").val() == "") {
            if (errorMessage == "")
                $("#eicName").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Eic Name can not be blank <br/>";
            i = i + 1;
            $("#eicName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnEicCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
