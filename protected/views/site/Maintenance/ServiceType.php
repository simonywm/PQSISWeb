<script>
$(function() {
    $("#aMenuMaintenanceLinkST").addClass("active");
        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>Service Type Id <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="serviceTypeId" name="serviceTypeId" type="text" class="form-control" placeholder=""
                            value="" readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    <div class="col-md-5">
                        <label> Service Type Name <span class="text-danger">*</span></label>
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateServiceType",
            type: "POST",
            cache: false,
    ...
    $("#serviceTypeId").val("<?php echo $this->viewbag['serviceType']['serviceTypeId']?>");
    $("#serviceTypeName").val("<?php echo $this->viewbag['serviceType']['serviceTypeName']?>");
    $("#showOrder").val("<?php echo $this->viewbag['serviceType']['showOrder']?>");
    $("#active").val("<?php echo $this->viewbag['serviceType']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertServiceType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#serviceTypeName").val() == "") {
            if (errorMessage == "")
                $("#serviceTypeName").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Service Type Name can not be blank <br/>";
            i = i + 1;
            $("#serviceTypeName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnServiceTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
