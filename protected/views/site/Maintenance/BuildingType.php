        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>Building Type Id <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="buildingTypeId" name="buildingTypeId" type="text" class="form-control" placeholder=""
                            value="" readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    <div class="col-md-5">
                        <label> Building Type Name <span class="text-danger">*</span></label>

                    </div>
                    <div class="col-md-7">
                        <input id="buildingTypeName" name="buildingTypeName" type="text" class="form-control"
                            placeholder="" value="">
                    </div>
                </div>
                <br />
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateBuildingType",
            type: "POST",
            cache: false,
    ...
    $("#buildingTypeId").val("<?php echo $this->viewbag['buildingType']['buildingTypeId']?>");
    $("#buildingTypeName").val("<?php echo $this->viewbag['buildingType']['buildingTypeName']?>");
    $("#active").val("<?php echo $this->viewbag['buildingType']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertBuildingType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#buildingTypeName").val() == "") {
            if (errorMessage == "")
                $("#buildingTypeName").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Building Type Name can not be blank <br/>";
            i = i + 1;
            $("#buildingTypeName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnBuildingTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
