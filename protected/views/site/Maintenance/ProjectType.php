        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>Project Type Id <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="projectTypeId" name="projectTypeId" type="text" class="form-control" placeholder=""
                            value="" readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    ...
                    <div class="col-md-7">
                        <input id="projectTypeName" name="projectTypeName" type="text" class="form-control"
                            placeholder="" value="">
                    </div>
                </div>
                <br />
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateProjectType",
            type: "POST",
            cache: false,
    ...
    $("#projectTypeId").val("<?php echo $this->viewbag['projectType']['projectTypeId']?>");
    $("#projectTypeName").val("<?php echo $this->viewbag['projectType']['projectTypeName']?>");
    $("#active").val("<?php echo $this->viewbag['projectType']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertProjectType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#projectTypeName").val() == "") {
            ...
            errorMessage = errorMessage + "Error " + i + ": " + "Project Type Name can not be blank <br/>";
            i = i + 1;
            $("#projectTypeName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnProjectTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
