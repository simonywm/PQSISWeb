        ...
        <form id="Form" onsubmit=>
            ...
            <div class="card-body">
                <div class="row" id="idDiv">
                    <div class="col-md-5">
                        <label>Business TypeId <span class="text-danger">*</span></label>
                    </div>
                    <div class="col-md-7">
                        <input id="businessTypeId" name="businessTypeId" type="text" class="form-control" placeholder=""
                            value="" readonly />
                    </div>
                </div>
                <br />
                <div class="row">
                    <div class="col-md-5">
                        <label> Business Type Name <span class="text-danger">*</span></label>

                    </div>
                    <div class="col-md-7">
                        <input id="businessTypeName" name="businessTypeName" type="text" class="form-control"
                            placeholder="" value="">
                    </div>
                </div>
                <br />
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateBusinessType",
            type: "POST",
            cache: false,
    ...
    $("#businessTypeId").val("<?php echo $this->viewbag['businessType']['businessTypeId']?>");
    $("#businessTypeName").val("<?php echo $this->viewbag['businessType']['businessTypeName']?>");
    $("#active").val("<?php echo $this->viewbag['businessType']['active']?>");
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertBusinessType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#businessTypeName").val() == "") {
            if (errorMessage == "")
                $("#businessTypeName").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Business Type Name can not be blank <br/>";
            i = i + 1;
            $("#businessTypeName").addClass("invalid");
        }
        if ($("#active").val() == "") {
            if (errorMessage == "")
                $("#active").focus();
    ...
    /* $("#btnBusinessTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
