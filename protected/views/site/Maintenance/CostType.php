<script>
$(function() {
    $("#aMenuMaintenanceLinkCT").addClass("active");
    ...
    <div class="card border border-secondary">
        <form id="Form" onsubmit=>
            <div class="card-header">
                <div class="row">
                    <div class="col-lg-8">
                        <h3 id="title"> Unit Rate </h3>
                        <label>Year&nbsp;:&nbsp; </label><input id="countYear" type="number" name="countYear"
                            style="width:20%" class="" placeholder="YYYY" value="">
                    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><input type="text" class="form-control"
                                        value="<?php echo $serviceType['serviceTypeName'] ?>" readonly disabled /></td>
                                <td>
                                    <div class="input-group mb-3">
                                        ...
                                        <input type="number" min="0" class="form-control"
                                            name="unitRate[<?php echo $serviceType['serviceTypeId'] ?>]" value="">
                                    </div>
                                </td>
                            </tr>
                            <?php }?>
...
$(document).ready(function() {
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxUpdateCostType",
            type: "POST",
            cache: false,
    ...
    $("#btnApply").unbind().bind("click", function() {
        ...
        $.ajax({
            url: "<?php echo Yii::app()->request->baseUrl; ?>/index.php?r=Maintenance/AjaxInsertCostType",
            type: "POST",
            cache: false,
            data: $("#Form").serializeArray(),
            success: function(data) {
    ...
    function validateInput() {
        ...
        if ($("#countYear").val() == "") {
            if (errorMessage == "")
                $("#countYear").focus();
            errorMessage = errorMessage + "Error " + i + ": " + "Count Year can not be blank <br/>";
            i = i + 1;
            $("#countYear").addClass("invalid");
        } else {
            <?php if( $this->viewbag['mode']!='update'){ ?>
            <?php foreach($this->viewbag['countYear'] as $countYear){ ?>
            if ($("#countYear").val() == "<?php echo $countYear['countYear']?>") {
                if (errorMessage == "")
                    $("#countYear").focus();
                errorMessage = errorMessage + "Error " + i + ": " +
                    "Required Year has been inserted,Please go update page for update <br/>";
                i = i + 1;
                $("#countYear").addClass("invalid");
            }
    ...
    /* $("#btnCostTypeCal").unbind().bind("click", function () {
         $("#loading-modal").modal("show");
         $(this).attr("disabled", true);
